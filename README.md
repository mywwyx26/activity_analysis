...
update: all files done!
- fiji avg has been moved to avg_clahe_binarize.py
- the only part that is manual is napari neuropils

pipeline:  
```mermaid
graph LR
    A[register] --> B[removebg.py]
    B --> C[avg_clahe_binarize.py]
    C --> D[napari neuropils]
    D --> E[activity.py]
    B --> F[clustering.py]
``` 
...
graph LR
    A[register] --> B[removebg.py]
    B --> C[fiji avg]
    C --> D[clahe_binarize.py]
    D --> E[napari neuropils]
    E --> F[activity.py]
    B --> G[clustering.py]
``` 
